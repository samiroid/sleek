...
def test_join():
	...
	assert status.JOIN_SURVEY_FAIL.format(bad_survey_id) in ret
	...
	assert status.JOIN_SURVEY_OK.format(survey_id, "10:00:00", "17:00:00") == ret
	data = backend.list_surveys(DB_path, silvio)[0]
	assert data[0] == silvio
	assert data[1] == survey_id
	#joining the same survey again
	ret = sleek.join_survey(["join",survey_id, am_check, pm_check], context) 
	assert ret == status.SURVEY_IS_SUBSCRIBED.format(survey_id)
...
def test_leave():
	...
	assert ret == status.LEAVE_SURVEY_FAIL.format(bad_survey_id)
	...
	assert ret == status.LEAVE_SURVEY_OK.format(survey_id)
...
def test_reminder():
	...
	assert ret == status.REMIND_SURVEY_OK.format(survey_id, "06:00:00", "18:20:00")
